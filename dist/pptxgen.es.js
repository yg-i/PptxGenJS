...
function genShadowEffectXml(shadow) {
  ...
  const frag = fragment().ele("a:effectLst").ele(shadowElementName, shadowAttrs).ele("a:srgbClr", { val: computed.color }).ele("a:alpha", { val: String(computed.opacity) }).up().up().up().up();
...
var ImageSizingXml = {
  cover: function(imgSize, boxDim) {
    ...
    return fragment().ele("a:srcRect", { l: String(hzPerc), r: String(hzPerc), t: String(vzPerc), b: String(vzPerc) }).up().ele("a:stretch").up().toString({ prettyPrint: false });
  },
  ...
  crop: function(imgSize, boxDim) {
    ...
    return fragment().ele("a:srcRect", { l: String(lPerc), r: String(rPerc), t: String(tPerc), b: String(bPerc) }).up().ele("a:stretch").up().toString({ prettyPrint: false });
  }
};
...
function makeXmlContTypes(slides, slideLayouts, masterSlide) {
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("Types", { xmlns: NS_CONTENT_TYPES });
...
function makeXmlRootRels() {
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("Relationships", { xmlns: NS_RELATIONSHIPS }).ele("Relationship", {
    Id: "rId1",
...
function makeXmlApp(slides, company) {
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("Properties", { xmlns: NS_EXTENDED_PROPERTIES, "xmlns:vt": NS_VT });
...
function makeXmlCore(title, subject, author, revision) {
  ...
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("cp:coreProperties", {
    "xmlns:cp": NS_CP,
    "xmlns:dc": NS_DC,
...
function makeXmlPresentation(pres) {
  ...
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("p:presentation", rootAttrs);
...
function makeXmlPresProps() {
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("p:presentationPr", {
    "xmlns:a": NS_A,
    "xmlns:r": NS_R,
    "xmlns:p": NS_P
  });
...
function makeXmlTableStyles() {
  const doc = create({ version: "1.0", encoding: "UTF-8", standalone: "yes" }).ele("a:tblStyleLst", {
    "xmlns:a": NS_A,
